...
# Crawl ~ Crawl
for tweet in tweepy.Cursor(api.search, q=query, tweet_mode="extended", count = 500,
                            lang = "id",untill = "2021-06-28",since = "2020-06-19").items(n):
...
data_json.to_json(f'Dataset/JRX {datetime.today().strftime("%Y-%m-%d")}.json')
...
data.to_csv(f'Dataset/JRX {datetime.today().strftime("%Y-%m-%d")}.csv', index=False)